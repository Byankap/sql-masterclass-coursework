...
SELECT
  ...
  SUM(CASE WHEN t.txn_type = 'BUY' THEN t.quantity
           WHEN t.txn_type = 'SELL' THEN t.quantity * -1
      END) AS total_quantity 
FROM trading.transactions t
JOIN trading.members m ON m.member_id = t.member_id
WHERE ticker = 'BTC'
...
SELECT
  members.region,
  AVG(CASE WHEN transactions.txn_type = 'BUY' THEN transactions.quantity
         WHEN transactions.txn_type = 'SELL' THEN transactions.quantity * -1
     END) * cte_latest_price.price AS avg_ethereum_value
FROM trading.transactions
INNER JOIN cte_latest_price ON transactions.ticker = cte_latest_price.ticker
INNER JOIN trading.members ON members.member_id = transactions.member_id
WHERE transactions.ticker = 'ETH'
GROUP BY members.region, cte_latest_price.price
ORDER BY avg_ethereum_value DESC;
```
